...
- 주요 엔티티:
  ...
  PlaceEntity(PLACE, 공간 공통: 이름·주소·타입 PlaceType(OFFICE/STATION/WORK_STAY)·검수상태 ApprovalState·호스트 FK·대표이미지),

  OfficeEntity(OFFICE, 오피스 상세: 운영시간·좌석수·시간단위 요금),

  StationEntity(WORK_STATION, 워크앤스테이 상세: 체크인/체크아웃 시간·객실수·일단위 요금·업무공간 정보),

  WorkStayEntity(WORK_STAY, 숙소 상세: 체크인/체크아웃 시간·객실수·일단위 요금) — Place 1:1

  PlaceImgEntity(PLACE_IMG, 다중 이미지 S3 URL·정렬순서),

  AmenityEntity(AMENITY, 편의시설 마스터) / PlaceAmenityEntity(PLACE_AMENITY, M:N 브리지),

  WishListEntity(WISH_LIST, 찜: member FK + place FK, unique 조합)

- 핵심 기능:

  공간 등록(호스트, 타입별 상세정보 + 다중이미지 S3 업로드 + 편의시설 다중선택) ·

  공간 수정(PATCH 부분 수정: null=유지 / 빈문자열=제거 규칙 일관 적용, 이미지 추가/삭제, 편의시설 갱신) ·

  어드민 검수(승인/반려, 반려 시 사유 필수, 반려 후 호스트 재신청 가능) ·

  공간 목록·상세 조회(타입별·승인된 것만 노출, 호스트 본인 미승인 공간 별도 조회) ·

  찜 등록·해제(토글 방식, 중복 방지) · 내 찜 목록 조회 ·

  편의시설 마스터 관리(어드민)

- 핵심 로직(등록·수정·검수·찜):

  공간 등록 ─ ①호스트 자격 검증(assertApproved — 8.1 회원/인증 게이트 호출, 미승인 호스트는 403)

    → ②공통 정보(Place) 저장 → ③타입별 상세(Office/Station/WorkStay) 저장

    → ④이미지 S3 업로드 후 PlaceImg 다중 저장 → ⑤편의시설 M:N 매핑 저장

    → 초기 검수상태 P(Pending). 어드민 승인 전까지 사용자 검색·예약에 노출되지 않음.

  공간 수정 ─ PUT 규칙(null=유지 / 빈문자열=제거)으로 부분 수정.

    이미지·편의시설은 별도 엔드포인트(추가/삭제). 수정 후 재검수 정책은 정책 결정 따름.

  어드민 검수 ─ P→A(승인) / P→R(반려, 사유 필수). 승인된 공간만 검색·예약 대상.

    반려 시 호스트가 정보 보완 후 재신청 → 다시 P로 전이.

  찜 토글 ─ (member, place) unique. 이미 있으면 DELETE, 없으면 INSERT.

    동시 클릭 시 unique 제약으로 멱등 보장. 찜 목록은 최신순 정렬.

- 미완성/이관:

  공간 예약 가능 여부 판단(블랙아웃·중복예약): 8.3 예약 도메인(보현)이 조회 — 공간은 마스터 데이터만 공급 ·

  공간 결제 수수료 산정(타입별 차등): 8.2 결제 도메인(우영)이 계산 — 공간 타입만 공급 ·

  공간 통계(조회수·예약전환율): 향후 과제 보류
```
