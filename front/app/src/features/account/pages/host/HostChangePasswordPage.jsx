...
import { changeHostPassword } from '../../api/hostApi';
...
function HostChangePasswordPage() {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { data: profile } = useHostMyPage(); // 호스트 본인 이메일 (인증 대상)
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await changeHostPassword(currentPw, newPw);
  ...
  return (
    ...
    >
      <CardStack>
        ...
        <ButtonRow>
          <GhostBtn onClick={() => navigate('/host/profile')}>취소</GhostBtn>{' '}
