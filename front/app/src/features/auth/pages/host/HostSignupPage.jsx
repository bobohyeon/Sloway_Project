...
const sectionTitle = {
  fontSize: 11, fontWeight: 600, color: '#A8B89F',
  letterSpacing: '0.5px', marginBottom: 12, marginTop: 4,
  paddingBottom: 6, borderBottom: '1px solid #e8efe5',
};
...
function Step1({ onNext }) {
  return (
    <>
      <AgreeBox>
        <AgreeAllRow><CheckboxWrap><input type="checkbox" /><span>전체 동의합니다</span></CheckboxWrap></AgreeAllRow>
        {TERMS.map((t) => (
          <AgreeItem key={t.id}>
            <AgreeItemLeft>
              <input type="checkbox" style={{ width: 15, height: 15, accentColor: '#A8B89F', cursor: 'pointer' }} />
              <span style={{ fontSize: 12 }}><Badge $req={t.required}>{t.required ? '필수' : '선택'}</Badge>{' '}{t.label}</span>
            </AgreeItemLeft>
      ...
      <AuthFooter>이미 호스트이신가요?{' '}<AuthLink as={Link} to="/login">로그인</AuthLink></AuthFooter>
    </>
  );
}
...
function Step2({ onNext, onPrev }) {
  return (
    <>
      ...
      <FormGroup>
        <FormLabel>상호명 <span style={{ color: '#e24b4a' }}>*</span></FormLabel>
        <FormInput type="text" placeholder="예) 청평 힐링 스테이" />
      </FormGroup>
      <FormGroup>
        <FormLabel>사업자등록번호 <span style={{ color: '#e24b4a' }}>*</span></FormLabel>
        <FormInput type="text" placeholder="000-00-00000" maxLength={12} />
      </FormGroup>
      <FormGroup>
        <FormLabel>사업자등록증 <span style={{ color: '#e24b4a' }}>*</span></FormLabel>
        <div style={{ display: 'flex', gap: 8 }}>
          <div style={{ flex: 1, padding: '11px 14px', border: '1px solid #ddd8cc', borderRadius: 8, fontSize: 13, color: '#c4bdb0', background: '#fff' }}>
            파일을 선택해주세요 (PDF, JPG, PNG)
          </div>
          <label style={{ padding: '10px 14px', background: '#A8B89F', color: '#fff', borderRadius: 8, fontSize: 12, fontWeight: 500, cursor: 'pointer', flexShrink: 0 }}>
            파일 선택<input type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display: 'none' }} />
          </label>
        </div>
        <p style={{ fontSize: 11, color: '#9a9280', marginTop: 4 }}>승인 검토에 사용되며 외부에 공개되지 않아요.</p>
      </FormGroup>
      <InfoBox>
        <strong style={{ display: 'block', marginBottom: 4 }}>호스트 승인 안내</strong>
        신청 후 영업일 기준 1~3일 내 검토 후 승인 결과를 이메일로 안내드려요.
      </InfoBox>
      <BtnPrimary type="button" onClick={onNext}>호스트 신청</BtnPrimary>
  ...
  return (
    <>
      <div style={{ textAlign: 'center', marginBottom: 20 }}>
        <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#e8efe5', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
        ...
        <h2 style={{ fontSize: 16, fontWeight: 600, color: '#2c2a22', marginBottom: 8 }}>호스트 신청이 완료됐어요!</h2>
        <p style={{ fontSize: 13, color: '#9a9280', lineHeight: 1.7 }}>영업일 기준 1~3일 내 검토 후<br />이메일로 결과를 안내드려요.</p>
      </div>
      <BtnPrimary type="button" onClick={() => navigate('/login')}>로그인하러 가기</BtnPrimary>
      <BtnSecondary type="button" onClick={() => navigate('/')}>메인으로</BtnSecondary>
    </>
  );
}
